  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        style={{ position: "fixed", left: 0, right: 0, top: "6vh", height: "46vh", zIndex: 55, pointerEvents: "none", display: "flex", alignItems: "center", justifyContent: "center", background: "radial-gradient(circle at center, rgba(0,0,0,0.22), rgba(0,0,0,0.08), rgba(0,0,0,0))", overflow: "hidden" }}
        initial={{ opacity: 0 }}
...
function HitCard({ hit, onHit, hitFx, clearHitFx, vanishFx, compact = false, ultraCompact = false, disableClicks = false }) {
  ...
  return (
    <motion.div key={hit.id} layout initial={{ opacity: 0, scale: 0.96, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.6, y: -10, filter: "blur(8px)" }} transition={{ type: "spring", stiffness: 260, damping: 24 }}>
      <div onClick={() => !disableClicks && onHit(hit.id)} style={{ position: "relative", overflow: "hidden", borderRadius: ultraCompact ? 10 : compact ? 12 : 16, border: style.border, backdropFilter: "blur(6px)", background: soldOut ? "rgba(55,65,81,0.15)" : "rgba(5,10,25,0.15)", boxShadow: `${style.glow}, 0 0 40px rgba(168,85,247,0.18)${hitFx ? ", 0 0 60px rgba(255,255,255,0.35)" : ""}`, cursor: soldOut || disableClicks ? "default" : "pointer", userSelect: "none", opacity: soldOut ? 0.88 : 1, pointerEvents: disableClicks ? "none" : "auto" }}>
        ...
        <motion.div animate={hitFx ? { scale: [1, 1.15, 0.92, 1.08, 1], rotate: [0, -2, 2, -1, 0] } : { scale: 1, rotate: 0 }} transition={{ duration: 0.42, ease: "easeOut" }}>
          ...
          <div style={{ padding: ultraCompact ? 6 : compact ? 8 : 12, background: soldOut ? "rgba(20,20,20,0.3)" : "rgba(0,0,0,0.15)" }}>
