...
/**
 * PokePlanet Live Wall — Supabase-ready single file
 *
 * Modes:
 * - default / ?mode=operator  -> operator controls visible
 * - ?mode=display            -> clean OBS view, no controls
 * - ?mode=mobile             -> narrow mobile preview
 * - ?mode=stream             -> framed stream test with controls visible
 *
 * Setup still required in Supabase dashboard:
 * 1. Create table: wall_sessions
 * 2. Columns:
 *    - id (text, primary key)
 *    - state (jsonb)
 *    - updated_at (timestamptz, default now())
 * 3. Realtime enabled for wall_sessions
 * 4. For initial testing, RLS disabled on wall_sessions
 */
...
const STORAGE_KEY = "pokeplanet-live-wall-fallback-state-v1";
const BASE_HITS_KEY = "pokeplanet-live-wall-fallback-base-v1";
...
const DEMO_HITS = [
  {
    id: "1",
    title: "MEGA LATIAS EX BOX",
    qty: 4,
    tier: "Chase",
    image: "https://images.pokemontcg.io/swsh11/115_hires.png",
  },
  {
    id: "2",
    title: "ASCENDED HEROES DELUXE PIN COLLECTION",
    qty: 6,
    tier: "Chase",
  ...
  {
    id: "3",
    title: "10X PACK LOT – ASCENDED HEROES",
    qty: 2,
    tier: "Standard",
    image: "https://res.cloudinary.com/dkspvhppa/image/upload/v1775347736/singer_byskqp.jpg",
  },
  {
    id: "4",
  ...
  {
    id: "9",
    title: "MEGA CHARIZARD UPC",
    qty: 4,
    tier: "Chase",
    image: "https://images.pokemontcg.io/swsh9/154_hires.png",
  },
];
...
const tierStyles = {
  Standard: {
    ...
    badge: "rgba(34,211,238,0.22)",
    glow: "0 0 16px rgba(34,211,238,0.18)",
    qtyColor: "#22d3ee",
  },
  Premium: {
    border: "1px solid rgba(217,70,239,0.5)",
    badge: "rgba(217,70,239,0.22)",
    glow: "0 0 18px rgba(217,70,239,0.2)",
    qtyColor: "#e879f9",
  },
  Chase: {
    border: "1px solid rgba(251,191,36,0.65)",
    badge: "rgba(251,191,36,0.24)",
    glow: "0 0 20px rgba(251,191,36,0.22)",
    qtyColor: "#facc15",
  },
};
...
function sortHitsForStream(hits) {
...
function runTests() {
  ...
  if (next.find((item) => item.id === "a")?.qty !== 1) {
    throw new Error("applyHitToHits should decrement quantity");
  }

  const soldOut = applyHitToHits(sample, "b");
  if (soldOut.find((item) => item.id === "b")?.qty !== 0) {
    throw new Error("applyHitToHits should keep sold-out items at qty 0");
  }

  const sorted = sortHitsForStream([
    { id: "2", title: "B", qty: 0, tier: "Standard", image: "" },
    { id: "1", title: "A", qty: 2, tier: "Standard", image: "" },
  ]);
  if (sorted[0].id !== "1") {
    throw new Error("sortHitsForStream should move sold-out items to the end");
  }

  const csv = `id,title,qty,tier,image\n1,Test Hit,3,chase,https://example.com/image.jpg`;
  const parsed = parseCsv(csv);
  if (parsed.length !== 1 || parsed[0].tier !== "Chase" || parsed[0].qty !== 3) {
    throw new Error("parseCsv should parse rows correctly");
  }
}
...
function BurstEffect({ onDone }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none", overflow: "hidden", borderRadius: 16, zIndex: 30 }}>
      <motion.div
        style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, rgba(253,224,71,0.35), rgba(251,146,60,0.18), rgba(232,121,249,0.22))" }}
        initial={{ opacity: 0 }}
        animate={{ opacity: [0, 0.9, 0.08, 0] }}
        transition={{ duration: 0.6, ease: "easeOut" }}
      />
      <motion.div
        style={{ position: "absolute", left: "50%", top: "50%", width: 64, height: 64, transform: "translate(-50%, -50%)", borderRadius: 9999, border: "2px solid rgba(254,240,138,0.95)" }}
      ...
      {particles.map((p) => (
        <motion.div
          key={p.id}
          style={{ position: "absolute", left: "50%", top: "50%", width: 10, height: 10, transform: "translate(-50%, -50%)", borderRadius: 9999, background: "rgba(254,240,138,1)", boxShadow: "0 0 18px rgba(253,224,71,1)" }}
...
function VanishEffect() {
  return (
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none", overflow: "hidden", borderRadius: 16, zIndex: 30 }}>
      <motion.div
        style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, rgba(253,224,71,0.28), rgba(232,121,249,0.18), rgba(103,232,249,0.22))" }}
        initial={{ opacity: 1 }}
        animate={{ opacity: 0 }}
        transition={{ duration: 0.5, ease: "easeOut" }}
      />
...
function ConfettiLayer({ trigger }) {
  const pieces = Array.from({ length: 32 }).map((_, i) => ({
    ...
    rotateEnd: 540 + Math.random() * 540,
    sway: 30 + Math.random() * 45,
    burstY: 6 + Math.random() * 12,
  }));
      ...
      {pieces.map((p) => (
        <motion.div
          ...
          style={{
            ...
            borderRadius: 2,
            background: `linear-gradient(135deg, hsl(${p.hue}, 95%, 68%), hsl(${(p.hue + 35) % 360}, 95%, 58%))`,
            boxShadow: `0 0 12px hsla(${p.hue}, 95%, 65%, 0.45)`,
          }}
          initial={{ x: 0, y: -20, opacity: 0, rotate: p.rotateStart, scale: 0.7 }}
          animate={{
            x: [0, p.driftX * 0.35, -p.sway * 0.35, p.sway * 0.55, -p.sway * 0.25, p.driftX],
            y: [0, p.burstY, 24, 110, 280, 560, 900],
            opacity: [0, 1, 1, 1, 0.92, 0.75, 0],
            rotate: [p.rotateStart, p.rotateStart + 90, p.rotateStart - 110, p.rotateStart + 180, p.rotateEnd],
            scale: [0.7, 1.1, 1, 1, 0.95, 0.9, 0.85],
          }}
          transition={{ duration: p.duration, delay: p.delay, ease: "easeOut", times: [0, 0.08, 0.18, 0.38, 0.62, 0.82, 1] }}
        />
      ))}
    </div>
  );
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 9999,
        background: soldOut ? "rgba(229,231,235,0.96)" : "rgba(0,0,0,0.82)",
        color: soldOut ? "#6b7280" : color,
        fontWeight: 900,
        fontSize: 28,
        border: "1px solid rgba(255,255,255,0.16)",
        boxShadow: soldOut ? "0 2px 8px rgba(0,0,0,0.12)" : `0 0 12px ${color}, 0 0 28px ${color}`,
      }}
    >
      <AnimatePresence mode="wait">
        <motion.span
          ...
          style={{ textShadow: soldOut ? "none" : `0 0 12px ${color}, 0 0 30px ${color}` }}
        >
          x{qty}
        </motion.span>
      </AnimatePresence>
...
function HitCard({ hit, onHit, hitFx, clearHitFx, vanishFx, compact = false, ultraCompact = false }) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, scale: 0.6, y: -10, filter: "blur(8px)" }}
      transition={{ type: "spring", stiffness: 260, damping: 24 }}
    >
      <div
        onClick={() => onHit(hit.id)}
        style={{
          position: "relative",
          overflow: "hidden",
          ...
          boxShadow: `${style.glow}, 0 0 40px rgba(168,85,247,0.18)${hitFx ? ", 0 0 60px rgba(255,255,255,0.35)" : ""}`,
          ...
          <div style={{ position: "relative", height: ultraCompact ? 96 : compact ? 120 : 180 }}>
            <img
              ...
              style={{
                ...
                objectFit: "contain",
                display: "block",
                background: soldOut ? "rgba(31,41,55,0.85)" : "rgba(0,0,0,0.6)",
                filter: soldOut ? "grayscale(1) brightness(0.75)" : "none",
              }}
            />
            ...
            <div
              style={{
                ...
                inset: 0,
                background: soldOut
                  ? "linear-gradient(to top, rgba(17,24,39,0.35), rgba(17,24,39,0.08), rgba(17,24,39,0))"
                  : "linear-gradient(to top, rgba(0,0,0,0.95), rgba(0,0,0,0.34), rgba(0,0,0,0.12))",
              }}
            />

            {!compact && (
              <div style={{ position: "absolute", top: 8, left: 8 }}>
                <span style={pillStyle(style.badge)}>{hit.tier}</span>
              </div>
            )}
...
export default function PokePlanetLiveWall() {
  ...
  const visibleHits = useMemo(() => (isStreamMode ? sortHitsForStream(hits) : hits), [hits, isStreamMode]);

  const saveFallbackState = (nextHits, nextBaseHits = null) => {
    if (!isBrowser) return;
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(nextHits));
    if (nextBaseHits) {
      window.localStorage.setItem(BASE_HITS_KEY, JSON.stringify(nextBaseHits));
    }
  };

  const pushStateToSupabase = async (nextHits, nextBaseHits = null) => {
    const payload = {
      id: SESSION_ID,
      state: {
        hits: nextHits,
  ...
  const syncState = async (nextHits, nextBaseHits = null) => {
    saveFallbackState(nextHits, nextBaseHits);
    try {
      await pushStateToSupabase(nextHits, nextBaseHits);
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      if (!hydrated) {
        try {
          const savedHits = window.localStorage.getItem(STORAGE_KEY);
          const savedBaseHits = window.localStorage.getItem(BASE_HITS_KEY);
          if (savedHits) {
            const parsedHits = normalizeHits(JSON.parse(savedHits));
      ...
      const channel = supabase
        .channel("pokeplanet-wall-session")
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "wall_sessions", filter: `id=eq.${SESSION_ID}` },
          (payload) => {
  ...
  useEffect(() => {
    if (!isBrowser) return;
    saveFallbackState(hits, baseHits);
  }, [hits, baseHits, isBrowser]);
  ...
  const updateColumns = async (value) => {
    setColumns(value);
    try {
      await pushStateToSupabase(hits, baseHits);
    } catch (err) {
      console.error(err);
    }
  };
    ...
    >
      ...
      <div style={{ maxWidth: isMobilePreview ? 430 : isDisplayMode ? 860 : 1800, margin: "0 auto" }}>
            ...
            >
              ...
              >
                ...
                <div style={{ fontSize: 18, color: "#cbd5e1", marginTop: 6 }}>
                  Total Hits Left: <span style={{ color: "#67e8f9", fontWeight: 800 }}>{totalRemaining}</span>
                </div>
                <div style={{ fontSize: 13, color: "#cbd5e1", marginTop: 8 }}>
                  Sync: <strong>{syncStatus}</strong>
                </div>
                <div style={{ fontSize: 13, color: "#cbd5e1", marginTop: 4 }}>
                  Modes: <strong>?mode=display</strong> for OBS, <strong>?mode=mobile</strong> for phone preview
                </div>
              </div>
              ...
              <button
                ...
                style={{
                  ...
                  padding: "12px 16px",
                  cursor: "pointer",
                  boxShadow: "0 6px 20px rgba(34,211,238,0.22)",
                }}
              >
                Reset Wall
              </button>
            ...
            {!isMobilePreview && (
              ...
              >
                <input
                  ...
                  placeholder="Paste your published Google Sheet CSV URL"
                  style={{
                    width: "100%",
                    borderRadius: 12,
                ...
                <button
                  ...
                  style={{
                    border: 0,
                    borderRadius: 12,
                    background: isLoading ? "#64748b" : "#a855f7",
                    color: "white",
                    fontWeight: 900,
                    fontSize: 14,
                    padding: "12px 16px",
                ...
                >
                  {isLoading ? "Loading..." : "Reload Sheet"}
                </button>
        ...
        <div
          style={{
            height: frameHeight,
            overflow: isStreamMode ? "hidden" : "visible",
          }}
        >
